...
void printDAG(Node *root, int level = 0) {
    if (root == NULL) return;
    ...
    while (!q.empty()) {
        ...
        if (curr->left != NULL) {
            Agnode_t *gLeft;
            if (strcmp(curr->left->value, "*") == 0 || strcmp(curr->left->value, "/") == 0 || 
                strcmp(curr->left->value, "-") == 0 || strcmp(curr->left->value, "+") == 0) {
                gLeft = agnode(g, strdup((std::string(curr->left->value) + "_" + std::to_string(counter++)).c_str()), 1);
            } else {
                gLeft = agnode(g, curr->left->value, 1);
            }
        ...
        if (curr->right != NULL) {
            Agnode_t *gRight;
            if (strcmp(curr->right->value, "*") == 0 || strcmp(curr->right->value, "/") == 0 || 
                strcmp(curr->right->value, "-") == 0 || strcmp(curr->right->value, "+") == 0) {
                gRight = agnode(g, strdup((std::string(curr->right->value) + "_" + std::to_string(counter++)).c_str()), 1);
            } else {
                gRight = agnode(g, curr->right->value, 1);
            }
